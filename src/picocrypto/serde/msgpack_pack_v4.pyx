...
cdef inline bint _is_type(object obj, object typ):
    """Exact type check using direct pointer comparison."""
    return (<PyObject*>obj).ob_type == <PyTypeObject*>typ
...
cdef void _msgpack_pack_obj(object obj, bytearray buf) except *:
    ...
    # Bool before int (bool is int subclass, must check first)
    if _is_type(obj, BOOL_TYPE):
        _append_byte(buf, 0xC3 if obj else 0xC2)
        return
    
    # Int and str are most common in typical workloads
    ...
    # Dict and list for nested structures
...
cpdef bytes msgpack_pack(object obj):
    """
    Pack obj to msgpack bytes (v3: optimized for real performance).
    
    Optimizations:
    - Pre-allocated buffer (512 bytes) to reduce early allocations
    - Direct type pointer comparison
    - Single resize for string/bytes header+data
    - Fast PyDict_Next and PyList_GET_ITEM iteration
    - Fused types for integer packing
    """
    cdef bytearray buf = bytearray(512)  # Pre-allocate to reduce resizes
    PyByteArray_Resize(buf, 0)  # Set length to 0 but keep capacity
